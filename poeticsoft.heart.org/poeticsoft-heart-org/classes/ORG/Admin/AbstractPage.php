...
use Poeticsoft\Heart\ORG\Validation\Validation;
...
abstract class AbstractPage {
	...
	public function register_settings() {
		...
		foreach ($this->settings as $setting) {
			$option_name = ORG::PREFIX . $setting['key'];
			...
			register_setting($this->slug, $option_name, [
				...
				'sanitize_callback' => function($value) use ($setting) {
					return ORG::get(Validation::class)->sanitize($value, $setting['field_type'] ?? 'text');
				},
	...
	public function render_field_callback($args) {
		$field_data = [
			'option_name' => ORG::PREFIX . $args['key'],
			'value'       => get_option(ORG::PREFIX . $args['key'], $args['value'] ?? ''),
			'type'        => $args['type'] ?? 'text',
	...
	protected function check_security() {
		if (! current_user_can($this->capability)) {
			wp_die(esc_html__('You do not have sufficient permissions to access this page.', ORG::TEXT_DOMAIN));
		}
	...
	protected function render_view($template_name, $data = []) {
		$data['page'] = $this;
		ORG::get(View::class)->render($template_name, $data);
	}
