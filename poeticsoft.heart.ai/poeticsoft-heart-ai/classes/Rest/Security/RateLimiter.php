...
namespace Poeticsoft\Heart\Rest\Security;
...
class RateLimiter {
	...
	public static function check_limit($action, $max_requests, $window_seconds) {
		...
		$key = AI::PREFIX . 'rl_' . $action . '_' . md5($ip);
		...
		if ($current_count >= $max_requests) {
			return new WP_Error(
				...
				__('Has excedido el límite de peticiones. Por favor, espera un momento.', 'poeticsoft-heart-ai'),
				['status' => 429]
			);
		}
