...
class Content extends Endpoint {
	...
	public function optimize_prompt(WP_REST_Request $request) {
		...
		if (!class_exists('Poeticsoft\Heart\AIAgent\AIAgent')) {
			return $this->response(false, 'El modulo AIAgent no está activo.', [], 500);
		}
		...
		try {
			$ai_agent = AI::get(AIAgent::class);
			// Note: If execute/generate_content is needed:
			// Adjusting call to match AIAgent logic.
			$params = [
				'system_prompt' => 'Optimiza el siguiente prompt de usuario para obtener mejores respuestas de una IA.',
				'user_prompt'   => $original_prompt,
			];
			
			$optimized_markdown = '';
			$ai_agent->execute($params, function($chunk) use (&$optimized_markdown) {
				$optimized_markdown .= $chunk;
			});
