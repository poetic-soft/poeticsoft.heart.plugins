...
abstract class AbstractPage
{
    ...
    public function register_settings()
    {
        ...
        foreach ($this->settings as $setting) {
            $option_name = Babel::PREFIX . $setting['key'];
            ...
            register_setting($this->slug, $option_name, [
                ...
                'sanitize_callback' => function ($value) use ($setting) {
                    return Babel::get(Validation::class)->sanitize($value, $setting['field_type'] ?? 'text');
                },
    ...
    {
        $field_data = [
            'option_name' => Babel::PREFIX . $args['key'],
            'value'       => get_option(Babel::PREFIX . $args['key'], $args['value'] ?? ''),
            'type'        => $args['type'] ?? 'text',
    ...
    protected function check_security()
    {
        if (! current_user_can($this->capability)) {
            wp_die(esc_html__('You do not have sufficient permissions to access this page.', Babel::TEXT_DOMAIN));
        }
    ...
    {
        $data['page'] = $this;
        Babel::get(View::class)->render($template_name, $data);
    }
