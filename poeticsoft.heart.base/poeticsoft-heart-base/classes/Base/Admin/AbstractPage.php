...
use Poeticsoft\Heart\Base\Validation\Validation;
...
abstract class AbstractPage {
	...
	public function register_settings() {
		...
		foreach ($this->settings as $setting) {
			$option_name = Base::PREFIX . $setting['key'];
			...
			register_setting($this->slug, $option_name, [
				...
				'sanitize_callback' => function($value) use ($setting) {
					return Base::get(Validation::class)->sanitize($value, $setting['field_type'] ?? 'text');
				},
	...
	public function render_field_callback($args) {
		$field_data = [
			'option_name' => Base::PREFIX . $args['key'],
			'value'       => get_option(Base::PREFIX . $args['key'], $args['value'] ?? ''),
			'type'        => $args['type'] ?? 'text',
	...
	protected function check_security() {
		if (! current_user_can($this->capability)) {
			wp_die(esc_html__('You do not have sufficient permissions to access this page.', Base::TEXT_DOMAIN));
		}
	...
	protected function render_view($template_name, $data = []) {
		$data['page'] = $this;
		Base::get(View::class)->render($template_name, $data);
	}
